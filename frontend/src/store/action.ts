...
import { adaptCommentToClient, adaptOffersToClient, adaptOfferToClient } from '../adapters/adapters-to-client';
import { OfferExtendedDto } from '../dto/offer/offer-extended-dto';
import { adaptCommentToServer, adaptOfferToServer, adaptUserToServer } from '../adapters/adapters-to-server';
...
export const fetchOffers = createAsyncThunk<Offer[], undefined, { extra: Extra }>(
  ...
  async (_, { extra }) => {
    ...
    return data.map((item) => adaptOffersToClient(item));
  });
...
export const fetchOffer = createAsyncThunk<Offer, Offer['id'], { extra: Extra }>(
  ...
  async (id, { extra }) => {
    ...
    try {
      const { data } = await api.get<OfferExtendedDto>(`${ApiRoute.Offers}/${id}`);
...
export const postOffer = createAsyncThunk<Offer, NewOffer, { extra: Extra }>(
  ...
  async (newOffer, { extra }) => {
    const { api, history } = extra;
    const { data } = await api.post<OfferExtendedDto>(ApiRoute.Offers, adaptOfferToServer(newOffer));
...
export const editOffer = createAsyncThunk<Offer, Offer, { extra: Extra }>(
  ...
  async (offer, { extra }) => {
    const { api, history } = extra;
    const { data } = await api.patch<OfferExtendedDto>(`${ApiRoute.Offers}/${offer.id}`, adaptOfferToServer(offer));
    history.push(`${AppRoute.Property}/${data.id}`);
...
export const postComment = createAsyncThunk<Comment, CommentAuth, { extra: Extra }>(
  ...
  async ({ id, comment, rating }, { extra }) => {
    const { api } = extra;
    const { data } = await api.post<CommentRdo>(`${ApiRoute.Offers}/${id}${ApiRoute.Comments}`, adaptCommentToServer({ comment, rating }));
